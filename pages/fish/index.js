...
const Fish = ({fishgame,navmenus}) => {
  ...
  return (
    <>
     ...
     <Head>
       <title>{fishgame.attributes.Title}</title>
        <meta name="description" content={fishgame.attributes.Description} />
        <link rel="canonical" href={`${fishgame.attributes.canonical}`} ></link>
     ...
     <NavBar navmenus={navmenus.items} />
      ...
      <Markdown>{fishgame.attributes.body}</Markdown>
      </div>
      </div>
...
export async function getServerSideProps() {
  ...
  //homes
  const res = await fetch(`${process.env.API_BASE_URL}/api/fishgames?`,{
  headers: { //เข้า API ผ่านรหัส
    'Content-Type': 'application/json',
    'Authorization': `Bearer ${process.env.BACKEND_API_KEY}`  
  },
...
const fishgames = await res.json()
if (fishgames.data.length === 0) {
  return {
      notFound: true,
  };
}
