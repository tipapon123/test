...
const Sport = ({sport,navmenus}) => {
  ...
  return (
    <>
        ...
        <Head>
        <title >{sport.attributes.Title}</title>
        <meta name="description" content={sport.attributes.Description} />
        <link rel="canonical" href={`${sport.attributes.canonical}`} ></link>
      ...
      <Markdown>{sport.attributes.body}</Markdown>
      </div>
      </div>
...
export async function getServerSideProps() {
  ...
  //homes
  const res = await fetch(`${process.env.API_BASE_URL}/api/sports?`,{
  headers: { //เข้า API ผ่านรหัส
    'Content-Type': 'application/json',
    'Authorization': `Bearer ${process.env.BACKEND_API_KEY}`  
  },
